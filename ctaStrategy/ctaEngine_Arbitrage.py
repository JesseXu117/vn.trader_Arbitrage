...
########################################################################
class CtaEngine_Arbitrage(object):   # by Jesse
    """CTA策略引擎"""
    settingFileName = 'CTA_Arbitrage_setting.json'   # by Jesse
    ...
    def processTradeEvent(self, event):
        ...
        if trade.vtOrderID in self.orderStrategyDict:
            ...
            # 计算策略持仓
            if trade.vtSymbol not in strategy.pos.keys():  # by Jesse
                strategy.pos[trade.vtSymbol] = 0  # by Jesse

            if trade.direction == DIRECTION_LONG:
                strategy.pos[trade.vtSymbol] += trade.volume  # by Jesse
            else:
                strategy.pos[trade.vtSymbol] -= trade.volume  # by Jesse
    ...
    def loadStrategy(self, setting):
        ...
        if name in self.strategyDict:
            ...
            for _symbol in vtSymbolList:         # by Jesse
                # print _symbol
                if _symbol in self.tickStrategyDict:
                    l = self.tickStrategyDict[_symbol]
                else:
                    l = []
                    self.tickStrategyDict[_symbol] = l
                l.append(strategy)

                # 订阅合约
                contract = self.mainEngine.getContract(_symbol)
                if contract:
                    req = VtSubscribeReq()
                    req.symbol = contract.symbol
                    req.exchange = contract.exchange
                    ...
                    self.writeCtaLog(u'%s的交易合约%s无法找到' %(name, _symbol))
