...
########################################################################
class CtaArbitrageTemplate(object):
    """CTA策略模板"""

    # 策略类的名称和作者
    className = 'CtaArbitrageTemplate'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        self.ctaEngine = ctaEngine  # by Jesse
        ...
        if setting:
            ...
            for key in self.paramList:
                if key in setting:
                    d[key] = setting[key]
                if key == 'vtSymbol':
                    self.vtSymbol = setting[key].split(',') # 此处对多合约的vtSymbol进行分割
    ...
    ########### combination order ###########
    # ----------------------------------------------------------------------
    def buyRiskReversal(self):
        """buy put, sell call and buy future组合单"""
        pass

    # ----------------------------------------------------------------------
    def sellRiskReversal(self):
        """buy call, sell put and sell future组合单"""
        pass

    # ----------------------------------------------------------------------
    def shortRiskReversal(self):
        """buy put, sell call and buy future组合单"""
        pass

    # ----------------------------------------------------------------------
    def coverRiskReversal(self):
        """buy call, sell put and sell future组合单"""
        pass
    ...
    def shortButterfly(self):
        """卖开蝶式套利组合单"""
        pass

    # ----------------------------------------------------------------------
    def coverButterFly(self):
        """买平蝶式套利组合单"""
        pass

    # ----------------------------------------------------------------------
    def buyStraddle(self, price, callsymbol, putsymbol, volume, stop=False):
        """买开跨式套利组合单"""
        # buy call option and put option
        if callsymbol[-4:] == putsymbol[-4:]:
            callOrderID = self.sendOrder(CTAORDER_BUY, price, volume, callsymbol, stop)
            putOrderID = self.sendOrder(CTAORDER_BUY, price, volume, putsymbol, stop)
        else:
            print u'buyStraddle: callsymbol与putsymbol行权价不一样！'
            return None
        return [callOrderID, putOrderID]

    # ----------------------------------------------------------------------
    def sellStraddle(self, price, callsymbol, putsymbol, volume, stop=False):
        """卖平跨式式套利组合单"""
        # sell call option and put option
        if callsymbol[-4:] == putsymbol[-4:]:
            callOrderID = self.sendOrder(CTAORDER_SELL, price, volume, callsymbol, stop)
            putOrderID = self.sendOrder(CTAORDER_SELL, price, volume, putsymbol, stop)
        else:
            print u'buyStraddle: callsymbol与putsymbol行权价不一样！'
            return None
        return [callOrderID, putOrderID]

    def shortStraddle(self, price, callsymbol, putsymbol, volume, stop=False):
        """卖开跨式式套利组合单"""
        # short call option and put option
        if callsymbol[-4:] == putsymbol[-4:]:
            callOrderID = self.sendOrder(CTAORDER_SHORT, price, volume, callsymbol, stop)
            putOrderID = self.sendOrder(CTAORDER_SHORT, price, volume, putsymbol, stop)
        else:
            print u'buyStraddle: callsymbol与putsymbol行权价不一样！'
            return None
        return [callOrderID, putOrderID]

    def coverStraddle(self, price, callsymbol, putsymbol, volume, stop=False):
        """买平跨式式套利组合单"""
        # cover call option and put option
        if callsymbol[-4:] == putsymbol[-4:]:
            callOrderID = self.sendOrder(CTAORDER_COVER, price, volume, callsymbol, stop)
            putOrderID = self.sendOrder(CTAORDER_COVER, price, volume, putsymbol, stop)
        else:
            print u'buyStraddle: callsymbol与putsymbol行权价不一样！'
            return None
        return [callOrderID, putOrderID]
    ...
    # ----------------------------------------------------------------------
    # ----------------------------------------------------------------------
    def arbbuy(self, price, volume, vtsymbol, stop=False):
        """套利订单买开"""
        return self.sendOrder(CTAORDER_BUY, price, volume, vtsymbol, stop)

    # ----------------------------------------------------------------------
    def arbsell(self, price, volume, vtsymbol, stop=False):
        """套利订单卖平"""
        return self.sendOrder(CTAORDER_SELL, price, volume, vtsymbol, stop)

    # ----------------------------------------------------------------------
    def arbshort(self, price, volume, vtsymbol, stop=False):
        """套利订单卖开"""
        return self.sendOrder(CTAORDER_SHORT, price, volume, vtsymbol, stop)

    # ----------------------------------------------------------------------
    def arbcover(self, price, volume, vtsymbol, stop=False):
        """套利订单买平"""
        return self.sendOrder(CTAORDER_COVER, price, volume, vtsymbol, stop)
    # ----------------------------------------------------------------------
    # ----------------------------------------------------------------------
...
########################################################################
class DataRecorder(CtaArbitrageTemplate):
    """
