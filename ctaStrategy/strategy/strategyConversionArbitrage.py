...
########################################################################
class ConversionStrategy(CtaArbitrageTemplate):
    """转换无风险套利策略"""
    strategyName = u'转换无风险套利策略'  # 策略实例名称
    className = u'ConversionStrategy'
    author = u'Jesse'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(ConversionStrategy, self).__init__(ctaEngine, setting)

        if setting:
            self.call_symbol = setting['callSymbol']
            self.put_symbol = setting['putSymbol']

        self.underlying = self.call_symbol[:5]

        self.K = int(self.call_symbol[-4:])
    ...
    def onTick(self, tick):
        ...
        if tick.vtSymbol == self.call_symbol:
            ...
            if self.bid_call - self.ask_put - 2 * self.option_fee  > self.ask_underly + self.future_fee - self.K:
                print 'positive conversion arbitrage: open position'
                size = size1
                ...
                print 'negenative conversion arbitrage: open position'
                size = size2
            ...
            if self.posDict[self.call_symbol] * self.posDict[self.put_symbol] < 0 and self.posDict[self.underlying]!=0:
                if self.ask_call - self.bid_put - 2 * self.option_fee  > self.bid_underly + self.future_fee - self.K:
                    ...
                    print 'positive conversion arbitrage: close position'
                    ...
                    print 'negenative conversion arbitrage: close position'
...
if __name__ == '__main__':
    ...
    # 在引擎中创建策略对象
    engine.initStrategy(ConversionStrategy, {})
